...
//						'value'=>substr($record['code'].str_repeat(' ',8),0,8).(empty($record['full_name'])?$record['name']:$record['full_name']),
	...
	public function actionAccountItemInEx($search, $acctid=0, $incity='') {
		...
		if ($acctid != 0) {
			$sql = "select b.rpt_cat from acc_account a, acc_account_type b
					...
					and a.city in ('99999','$city')
				";
			$row = Yii::app()->db->createCommand($sql)->queryRow();
			if ($row!==false) $type = $row['rpt_cat']=='CASH' ? 'CI' : 'BI';
		}
		echo $this->searchAccountItemEx($search, $type, $city);
	}

	public function actionAccountItemOutEx($search, $acctid=0, $incity='') {
		$type = 'ZZZ';
		$city = empty($incity) ? Yii::app()->user->city() : $incity;
		if ($acctid != 0) {
			$sql = "select b.rpt_cat from acc_account a, acc_account_type b
	...
	protected function searchAccountItemEx($search, $type, $city)
	{
		$suffix = Yii::app()->params['envSuffix'];
		$result = array();
		...
		$sql = "select a.code, concat(a.name,' (',a.code,')') as value, a.acct_code, b.name as acct_code_desc 
				from acc_account_item a left outer join acc_account_code b on a.acct_code=b.code and b.city='$city'
