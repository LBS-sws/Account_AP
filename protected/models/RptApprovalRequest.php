<?php
class RptApprovalRequest extends CReport {
	...
	public function retrieveUsers() {
		...
		$sql = "select username from security$suffix where a_read_only like '%XB06%'";
...
*/
		...
		$sql = "select 
				...
				from
					...
					inner join workflow$suffix.wf_state b on a1.current_state = b.id and b.code='PA'
					...
					left outer join account$suffix.acc_account_item j on i.field_value=j.code and e.city=j.city
...
//	}
	...
	public function submitEmail($user, $msg) {
		...
		$subject = Yii::t('report','Daily Request Approval Summary').' - '.General::toDate($date);
		$desc = Yii::t('report','Daily Request Approval Summary').' ('.Yii::t('report','User').': '.$user.') - '.General::toDate($date);
