...
class RptReimbursement extends CReport {
	...
	public function genReport() {
		...
		if (empty($ref_no)) {
			$sql = "select a.*, 
						workflow$suffix.RequestStatusDate(a.city,'PAYMENT',a.id,a.req_dt,'SI') as sts_dt1,
						workflow$suffix.RequestStatusDate(a.city,'PAYMENT',a.id,a.req_dt,'S') as sts_dt2,
						workflow$suffix.ActionPerson(a.city,'PAYMENT',a.id,a.req_dt,'PC') as acctstaff,
						workflow$suffix.ActionPerson(a.city,'PAYMENT',a.id,a.req_dt,'PS') as approver1,
						workflow$suffix.ActionPerson(a.city,'PAYMENT',a.id,a.req_dt,'PA') as approver2
					from acc_request a
					...
					or (workflow$suffix.RequestStatusDate(a.city,'PAYMENT',a.id,a.req_dt,'S')>='".General::toDate($start_dt)." 00:00:00' 
					and workflow$suffix.RequestStatusDate(a.city,'PAYMENT',a.id,a.req_dt,'S')<='".General::toDate($end_dt)." 23:59:59'))
				";
		} else {
			$sql = "select a.*, 
						workflow$suffix.RequestStatusDate(a.city,'PAYMENT',a.id,a.req_dt,'SI') as sts_dt1,
