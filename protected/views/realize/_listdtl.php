<?php
	$cls_str = "class='clickable-row' data-href='"
		.$this->getLink('XA06', 'realize/edit', 'realize/edit', array('index'=>$this->record['id']))
		."'";
?>
<tr>
...
<?php endif ?>
	<td <?php echo $cls_str;?>>
		<?php
			...
			echo TbHtml::hiddenField($this->getFieldName('payrealcountdoc'), $this->record['payrealcountdoc']);
	...
	<td>
		<?php 
			echo TbHtml::button($this->record['payrealcountdoc'], 
				array(
					'class'=>'btn-xs',
					'onclick'=>'javascript:showpayreal('.$this->record['id'].');',
				)
			);
		?>
	</td>
