...
class General {
	...
	*/
	...
	public static function getAcctCodeList($city)
	{
		$list = array();
		$sql = "select code, name from acc_account_code where city='$city' order by code";
		$rows = Yii::app()->db->createCommand($sql)->queryAll();
		if (count($rows) > 0) {
			foreach ($rows as $row) {
	...
	public static function getAcctItemList($city)
	{
		$list = array();
		$sql = "select code, name from acc_account_item where city='$city' order by code";
		$rows = Yii::app()->db->createCommand($sql)->queryAll();
		if (count($rows) > 0) {
			foreach ($rows as $row) {
	...
	public static function getTransTypeList($city,$type,$open=false,$adj=false)
	{
		$list = array();
		...
		$sql = "select trans_type_code, trans_type_desc 
				from acc_trans_type
				where trans_cat='$type' and city='$city' $clause1 $clause2  
	...
	public static function getCounterTransType($code, $city) {
		$sql = "select counter_type from acc_trans_type where trans_type_code='$code' and city='$city'";
